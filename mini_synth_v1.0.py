...
A simple, one-octave interactive piano keyboard built with Python's Tkinter library.
...
def main():
    ...
    window_width = len(WHITE_KEYS_NOTES) * WHITE_KEY_WIDTH
    ...
    # Create white keys
    for i, note in enumerate(WHITE_KEYS_NOTES):
        key = ttk.Button(keyboard_frame, text=note, style="White.TButton", command=lambda n=note: play_note(n))
        key.place(x=i * WHITE_KEY_WIDTH, y=0, width=WHITE_KEY_WIDTH, height=WHITE_KEY_HEIGHT)
        white_keys.append(key)

    # Create black keys
    for i, note in zip(BLACK_KEYS_POSITIONS, BLACK_KEYS_NOTES):
        ...
        key.place(x=(i * WHITE_KEY_WIDTH) + (WHITE_KEY_WIDTH - BLACK_KEY_WIDTH / 2), y=0, width=BLACK_KEY_WIDTH, height=BLACK_KEY_HEIGHT)
